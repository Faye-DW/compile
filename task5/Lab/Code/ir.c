...
static void mipsEmitFunction(FILE* out, MIPSFunc* f) {
    ...
    char endLabel[96];
    snprintf(endLabel, sizeof(endLabel), "%s_epilogue", f->name);
...
/* ================================================================
 * IR file I/O — read/write text-format IR to/from IRCode linked list
 * ================================================================ */

IRCode* readIRFromFile(const char* filename) {
    FILE* in = fopen(filename, "r");
    if (!in) {
        fprintf(stderr, "Error opening IR file: %s\n", filename);
        return NULL;
    }
...
void generateIR(ASTNode* root, const char* outputFile) {
    ...
    if (extDefList && nodeIs(extDefList, "ExtDefList")) {
        ...
        IRCode* optimized = optimizeIR(code);

        /* Detect output format: .s for MIPS assembly, default to IR text */
        int isMIPS = outputFile && strstr(outputFile, ".s") != NULL;
        if (isMIPS) {
            generateMIPS(optimized, out);
        } else {
            printIR(optimized, out);
        }
